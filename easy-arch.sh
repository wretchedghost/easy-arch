...
# Select the amount of space for swapfile (function).
swapfile_selector() {
    input_print "Please enter the amount of space to use as your swapfile in MB (ie. 4096 = 4 GB):"
    read -r -s swappifile
    if [[ $swappifile -lt 0 || $swappifile -gt 99999 ]]; then
        error_print "That is not a valid swapfile input, please try again."
        return 1
    fi
    return 0
}
...
# Create Swapfile
info_print "Creating a swapfile".
dd if=/dev/zero of=/mnt/swapfile bs=1M count=$swappifile status=progress &>/dev/null
...
pacstrap /mnt --needed base "$kernel" "$microcode" linux-firmware $kernel-headers grub rsync efibootmgr bash-completion screenfetch vim vi git &>/dev/null
...
HOOKS=(base udev autodetect keyboard consolefont modconf block encrypt filesystems)
EOF

# Setting up LUKS2 encryption in grub.
info_print "Setting up grub config."
UUID=$(blkid -s UUID -o value $CRYPTROOT)
sed -i "\,^GRUB_CMDLINE_LINUX=\"\",s,\",&rd.luks.name=$UUID=cryptroot root=$LUKS_SYS," /mnt/etc/default/grub
